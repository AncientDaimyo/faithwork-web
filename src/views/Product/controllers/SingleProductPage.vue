<template>
  ...
  <div class="single-product-page-wrapper">
    <div class="image-wrapper">
      ...
      <img src="../../../views/Shared/pic/FW_GIRLS_LOGO_black_2x3.png" alt="" class="single-product-image">
    </div>
    <div class="info">
      ...
      <form class="single-product-size-form" @submit.prevent="addToCart">
        <div class="single-product-size-wrapper">
          <div class="single-product-size-cycle" v-for="size in product.sizes" :key="size" :size="size">
            <input :id="`size-${size}`" type="radio" name="getProductSize" v-model="sizeSelected" :value="size"
              required />
  ...
  methods: {
    addToCart() {
      if (this.sizeSelected) {
        ...
        this.showContinueShoppingButton = true; // Отображаем кнопку "Продолжить покупки"
      } else {
    ...
    continueShopping() {
      this.$router.push({ name: 'shop' }); // Перенаправляем пользователя на страницу каталога
    },
  },
};
</script>